...
def test_execute_without_conversation(duckchat_model):
    ...
    with patch.object(DuckChat, 'fetch_vqd', return_value="test_vqd"), \
         patch.object(DuckChat, 'fetch_response') as mock_fetch_response:
        
        mock_response = MagicMock()
        mock_response.headers = {"x-vqd-4": "new_vqd"}
        mock_response.iter_lines.return_value = [b'data: {"message": "I am fine."}', b'data: [DONE]']
...
def test_execute_with_conversation(duckchat_model):
    ...
    conversation.responses = [
        MagicMock(prompt=MagicMock(prompt="What is your name?"), text=MagicMock(return_value="My name is DuckChat.")),
    ]
    ...
    with patch.object(DuckChat, 'fetch_vqd', return_value="test_vqd"), \
        ...
        mock_response = MagicMock()
        mock_response.headers = {"x-vqd-4": "new_vqd"}
        mock_response.iter_lines.return_value = [b'data: {"message": "It is sunny."}', b'data: [DONE]']
        mock_fetch_response.return_value = mock_response
        
        response = duckchat_model.execute(prompt, stream=False, response=MagicMock(), conversation=conversation)
...
def test_fetch_response_success():
    with patch('requests.post') as mock_post:
        ...
        response = DuckChat.fetch_response("https://example.com", "test_vqd", "gpt-4o-mini", [])
...
def test_fetch_response_failure():
    with patch('requests.post') as mock_post:
        ...
        with pytest.raises(Exception, match="Failed to send message: 400 Bad Request"):
            DuckChat.fetch_response("https://example.com", "test_vqd", "gpt-4o-mini", [])